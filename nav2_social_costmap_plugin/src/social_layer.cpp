...
void SocialLayer::onInitialize() 
{
  ...
  ppl_sub_ = node->create_subscription<people_msgs::msg::People>(
  ...
  std::cout<<ppl_sub_->get_topic_name();
...
void SocialLayer::get_parameters() {
  ...
  node->get_parameter(name_ + "." + "covariance_front_height",
                       sigma_front_height_);
  node->get_parameter(name_ + "." + "covariance_front_width",
                       sigma_front_width_);
  node->get_parameter(name_ + "." + "covariance_rear_height",
                       sigma_rear_height_);
  node->get_parameter(name_ + "." + "covariance_rear_width",
                       sigma_rear_width_);
  node->get_parameter(name_ + "." + "covariance_right_height",
                       sigma_right_height_);
  node->get_parameter(name_ + "." + "covariance_right_width",
                       sigma_right_width_);
  node->get_parameter(name_ + "." + "covariance_when_still",
                       sigma_when_still_);
  node->get_parameter(name_ + "." + "use_passing", use_passing_);
  node->get_parameter(name_ + "." + "use_vel_factor", use_vel_factor_);
  node->get_parameter(name_ + "." + "speed_factor_multiplier", speed_factor_);
...
void SocialLayer::updateBounds(double origin_x, double origin_y,
  ...
  std::string global_frame =layered_costmap_->getGlobalFrameID(); // Returns the global frame of the costmap
...
void SocialLayer::updateCosts(nav2_costmap_2d::Costmap2D & master_grid,
  ...
  for (p_it = transformed_people_.begin(); p_it != transformed_people_.end();
    ...
    if (dx < 0)
      start_x = -dx;
    else if (dx + width_cells > size_x)
      end_x = std::max(0, static_cast<int>(size_x) - dx);
    ...
    if (dy < 0)
      start_y = -dy;
    else if (dy + height_cells > size_y)
      end_y = std::max(0, static_cast<int>(size_y) - dy);
    ...
    for (int i = start_x; i < end_x; i++) {
      for (int j = start_y; j < end_y; j++) {
        int index = master_grid.getIndex(i + dx, j + dy);
      ...
      // Update the master_grid with the maximum value between the computed cost and the old cost
        master_array[index] = std::max(cvalue, old_cost);
